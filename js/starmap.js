...
$(window).on('load', function () {
  ...
  highways.children().each(function () {
    ...
    const secondVar = childId.substring(
      childId.indexOf('_') + 1,
      childId.lastIndexOf('_')
    );
...
function travelTo(planet) {
  ...
  const hasMatchingLocations = checkHighwayLocations(
    planet,
    sessionStorage.getItem('currentPlanet')
  );
  if (planet == sessionStorage.getItem('currentPlanet')) {
    ...
    hideLines();
    // Set New Planet
    document.getElementById(planet).children[0].style.outline =
      'rgba(0, 128, 0, 0.85) thick solid';
    ...
    alert(
      'You are out of fuel! You have no choice but to send out distress signals.'
    );
    setTimeout(function () {
      ...
      if (randInt == 1) {
        ...
        // Fuel donation
        alert(
          'After some time, a civilian vessel arrives - it takes pity on you and offers 1 fuel.'
        );
...
function worldSeed(sessionSeed) {
  ...
  sessionStorage.setItem('allFactionsPlanets', [
    faction1Planets,
    faction2Planets,
    faction3Planets,
    faction4Planets,
  ...
  // Ensure Spawn
  if (lastPos == null) {
    // If no session save found
    console.log('Save Not Found');
    switch (
      difficulty // Set spawn location based on faction select
    ) {
      case '1':
        forcetravelTo('Hymera');
        faction1Planets.splice(
          faction1Planets.findIndex((el) => el == 'Hymera'),
          1
        );
        break;
      case '2':
        forcetravelTo('Acheron');
        faction2Planets.splice(
          faction2Planets.findIndex((el) => el == 'Acheron'),
          1
        );
      ...
      case '3':
        ...
        faction3Planets.splice(
          faction3Planets.findIndex((el) => el == 'Meissa'),
          1
        );
  ...
  if (sessionSeed == null) {
    ...
    var seed = Math.floor(
      Math.random() *
        eval(
          'faction' + sessionStorage.getItem('difficulty') + 'Planets.length'
        ) +
        1
    );
    seed = 0; // Testing
  ...
  for (i = 0; i < sessionStorage.getItem('seed'); i++) {
    // Faction 1
...
function checkEvent(planet) {
  ...
  for (i = 1; i < 4; i++) {
    ...
    if (index == -1) {
      ...
      console.log(
        'Planet: ' + planet + '|' + 'Faction: ' + i + '|' + 'Event: ' + event
      );
      switch (event) {
        case 'battle':
        ...
        case 'asteroid':
          ...
          break;
        case 'trader':
          console.log('Nearby Trading Station!');
          debug('trader');
          sessionStorage.setItem('activeEvent', 'trader');
          break;
        default:
          break;
      }
